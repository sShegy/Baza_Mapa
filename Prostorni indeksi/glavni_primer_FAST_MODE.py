...
# --- PREKIDAČ ZA BRZI MOD ---
# Postavite na True da se simulacija završi za par sekundi.
# Postavite na False za normalnu, sporu, vizuelnu simulaciju.
FAST_MODE = True
# ---------------------------
...
def load_accidents_data():
    ...
    putanja_do_fajla_nezgoda = '../nez-opendata-2024-20250125.xlsx'
...
if __name__ == "__main__":
    ...
    # 7. Inicijalizuj simulator kretanja automobila
    # U brzom modu, brzina je 10000 km/h; u normalnom je 250 km/h
    brzina_simulacije = 10000 if FAST_MODE else 250
    automobil = AutoSimulator(route_coords, speed_kmh=brzina_simulacije, interval=1.0)
    ...
    try:
        ...
        while automobil.running:
            ...
            # Čekaj interval pre sledećeg pomeraja (samo ako nismo u brzom modu)
            if not FAST_MODE:
    ...
    # =============================================================================
    # === KAKO JE IMPLEMENTIRAN "BRZI REŽIM" (FAST MODE) ===
    # =============================================================================
    #
    # Da bi se omogućilo brzo testiranje, implementiran je brzi režim.
    # To je postignuto kroz sledeće tri izmene u kodu ovog fajla:
    #
    # 1. DODATA JE GLOBALNA PROMENLJIVA (PREKIDAČ):
    #    Na vrhu fajla, definisana je nova boolean promenljiva `FAST_MODE`.
    #    Ona služi kao centralni prekidač za celu funkcionalnost.
    #
    #       FAST_MODE = True  # ili False
    #
    # 2. USLOVNO PODEŠAVANJE BRZINE VOZILA:
    #    Linija koda gde se kreira objekat `automobil` je izmenjena. Umesto fiksne
    #    brzine, sada koristi uslovni izraz koji čita vrednost `FAST_MODE` i na
    #    osnovu nje postavlja brzinu na 10,000 km/h (za brzi mod) ili 250 km/h (za normalni).
    #
    #       brzina_simulacije = 10000 if FAST_MODE else 250
    #       automobil = AutoSimulator(route_coords, speed_kmh=brzina_simulacije, interval=1.0)
    #
    # 3. USLOVNO PAUZIRANJE SIMULACIJE:
    #    Na kraju glavne `while` petlje, poziv `time.sleep(interval_simulacije)`
    #    je ubačen u `if` blok. Pauza se sada izvršava samo ako `FAST_MODE` NIJE
    #    aktivan (tj. ako je postavljen na `False`).
    #
    #       if not FAST_MODE:
    #           time.sleep(interval_simulacije)
    #